...
ggplot(car_train, aes(x = mileage, y = log(usd))) +
  ...
  theme_minimal()
#scatterplot
ggplot(car_train, aes(x = miles, y = usd)) +
  geom_point() +
  geom_smooth()+
...
final_linear_res |> collect_metrics()
...
set.seed(2)
cls_fit <- brulee_mlp(
  usd ~ ., data = juice(prep(car_recipe)), 
...
predict(cls_fit, bake(prep(car_recipe),new_data=car_test)) %>%
  bind_cols(car_test$usd) %>%
  ggplot(aes(log(.pred), log(...2))) +
  geom_point()
```
