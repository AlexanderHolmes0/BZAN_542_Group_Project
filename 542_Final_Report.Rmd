...
$\hspace{20pt}$The results of the MLP trained with the BRLUEE engine were promising, as evidenced by the relatively low RMSE and high R-squared values. The model's performance was also consistent across the training and testing sets, indicating that it was not overfitting the data. The model predictions between both training and testing are displayed in Figure \@ref(fig:MLP-Predictions). The model predictions were relatively accurate with a training RMSE of `r show_best(NET_Race_res,metric='rmse',n=1)[,5]` and standard error of `r show_best(NET_Race_res,metric='rmse',n=1)[,7]` displayed in Table \@ref(tab:MLP-train). MLP testing results were marginally better with an RMSE of `r net_res[2,3]` and a R-Squared of `r net_res[2,3]` displayed in Table \@ref(tab:MLP-test).
...
$\hspace{20pt}$The results of the KNN model were promising, as evidenced by the relatively low RMSE and high R-squared values. The model's performance was also consistent across the training and testing sets, indicating that it was not overfitting the data. The model predictions between both training and testing are displayed in Figure \@ref(fig:KNN-Predictions). The model predictions were relatively accurate with a training RMSE of `r show_best(KNN_Work_Spec,metric='rmse',n=1)[,5]` and standard error of `r show_best(KNN_Work_Spec,metric='rmse',n=1)[,7]` displayed in Table \@ref(tab:KNN-train). KNN testing results were marginally better with an RMSE of `r knn_res[1,3]` and a R-Squared of `r knn_res[2,3]` displayed in Table \@ref(tab:KNN-test). The KNN model was a great leap forward in performance compared to the previous two models in Section \@ref(glmnet) and Section \@ref(mlp).
...
Model_Performance <- data.frame(ModelNames = c("GLMNET","MLP","KNN","Decision Tree","Light GBM","XGBoost"),
                                TrainError = c(show_best(car_results,metric='rmse',n=1)[,5],
                                               show_best(NET_Race_res,metric='rmse',n=1)[,5],
                                               show_best(KNN_Work_Spec,metric='rmse',n=1)[,5],
                                               show_best(DT_Train,metric='rmse',n=1)[,5],
                                               show_best(xgb_rs,metric='rmse',n=1)[,5],
                                               show_best(lgbm_race_res,metric='rmse',n=1)[,5]),
                                TestError = c(6282.8891859,5606.3147042,3145.1372420,3364.4609826,2842.0316563,2740.960997))
...
ggplot(data = Model_Performance,
  ...
  geom_point(size = 10) +
  scale_shape_manual(name = "Error Type", labels = c("Train Error","Test Error"), values=c(16,17))+
...
```{r}
all_model_training_Rmse <- c(7051.5063,6235.1162090,3146.8545379,3343.7587802,2946.1450871,2842.2375825)
all_model_training_std_err <- c(191.7094765,203.3696426,146.2410052,260.5655685,230.5682199,180.531215)


all_model_training_Rsq <- c(0.8005,0.8436827,0.9600549,0.9544921,0.9645135,0.9672529)
all_model_training_rsq_error <- c(0.0114633,0.0119983,0.0036643,0.0069266,0.0053957,0.003974)

all_model_testing_Rmse <- c(6282.8891859,5606.3147042,3145.1372420,3364.4609826,2842.0316563,2740.960997)
all_model_test_rqs <- c(0.8107064,0.8492034,0.9522852,0.9472024,0.9614465,0.964083)
