...
bind_rows(show_best(DT_Train,metric="rsq",n=1)) |> 
  select(.metric,mean,std_err),caption = "Decision Tree Training Results",booktabs = T) |> 
  kableExtra::kable_styling(latex_options = c("striped", "hold_position"),full_width = F)
```

```{r DT-test}
kableExtra::kbl(final_DT_res |> collect_metrics() |> filter(.metric %in% c('rmse','rsq')) |> 
  select(.metric,.estimate),caption = "Decision Tree Test Results", booktabs = T)|> 
  kableExtra::kable_styling(latex_options = c("striped", "hold_position"),full_width = F)
```
...
The KNN model is set up with specific tuning parameters. We select the best model based on the lowest RMSE, identified through a racing process. The top-tuned KNN model's predictions are displayed in a calibration plot. The final model, optimized with the best hyper-parameters, is tested on the testing dataset.\
